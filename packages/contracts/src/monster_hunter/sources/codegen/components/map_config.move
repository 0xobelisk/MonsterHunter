  ...
  module monster_hunter::monster_hunter_map_config {
  ...
  public fun new(width: u64, height: u64, terrain: vector<u32>): Map_config {
    Map_config {
            width,
            height,
            terrain,
  ...
  public fun terrain(self: &Map_config): vector<u32> {
    self.terrain
  }
  ...
  public fun update_terrain(self: &mut Map_config, terrain: vector<u32>) {
  ...
  public fun get_value_schemas(): vector<vector<u8>> {
    vector[b"u64", b"u64", b"vector<u32>"]
  }
  ...
  public fun get_value_names(): vector<vector<u8>> {
    vector[b"width", b"height", b"terrain"]
  }
  ...
  public fun has_terrain(dapp_hub: &DappHub): bool {
    let key_tuple = vector::empty();
    dapp_service::has_field<DappKey>(dapp_hub, get_table_id(), key_tuple, 2)
  }

  public fun ensure_has_terrain(dapp_hub: &DappHub) {
    let key_tuple = vector::empty();
    dapp_service::ensure_has_field<DappKey>(dapp_hub, get_table_id(), key_tuple, 2)
  }

  public fun ensure_not_has_terrain(dapp_hub: &DappHub) {
    let key_tuple = vector::empty();
  ...
  public fun get_terrain(dapp_hub: &DappHub): vector<u32> {
    let key_tuple = vector::empty();
    let value = dapp_service::get_field<DappKey>(dapp_hub, get_table_id(), key_tuple, 2);
    let mut bsc_type = sui::bcs::new(value);
    let terrain = sui::bcs::peel_vec_u32(&mut bsc_type);
    terrain
  }

  public fun set_terrain(dapp_hub: &mut DappHub, terrain: vector<u32>) {
    let key_tuple = vector::empty();
    let value = to_bytes(&terrain);
    dapp_service::set_field(dapp_hub, monster_hunter_dapp_key::new(), get_table_id(), key_tuple, 2, value);
  }

  public fun get(dapp_hub: &DappHub): (u64, u64, vector<u32>) {
    let key_tuple = vector::empty();
  ...
  public fun set(dapp_hub: &mut DappHub, width: u64, height: u64, terrain: vector<u32>) {
    let key_tuple = vector::empty();
    let value_tuple = encode(width, height, terrain);
  ...
  public fun encode(width: u64, height: u64, terrain: vector<u32>): vector<vector<u8>> {
    let mut value_tuple = vector::empty();
    value_tuple.push_back(to_bytes(&width));
    value_tuple.push_back(to_bytes(&height));
    value_tuple.push_back(to_bytes(&terrain));
    value_tuple
  }

  public fun encode_struct(map_config: Map_config): vector<vector<u8>> {
    encode(map_config.width, map_config.height, map_config.terrain)
  }
