  ...
  module monster_hunter::monster_hunter_monster_catch_attempt {
  ...
  const TABLE_NAME: vector<u8> = b"monster_catch_attempt";
  ...
  public fun get(dapp_hub: &DappHub, monster: address): MonsterCatchResult {
    let mut key_tuple = vector::empty();
    key_tuple.push_back(to_bytes(&monster));
    let value = dapp_service::get_field<DappKey>(dapp_hub, get_table_id(), key_tuple, 0);
    let mut bsc_type = sui::bcs::new(value);
    let value = monster_hunter::monster_hunter_monster_catch_result::decode(&mut bsc_type);
    value
  }

  public fun set(dapp_hub: &mut DappHub, monster: address, value: MonsterCatchResult) {
    let mut key_tuple = vector::empty();
    key_tuple.push_back(to_bytes(&monster));
  ...
  public fun encode(value: MonsterCatchResult): vector<vector<u8>> {
    let mut value_tuple = vector::empty();
    value_tuple.push_back(monster_hunter::monster_hunter_monster_catch_result::encode(value));
    value_tuple
  }
}
